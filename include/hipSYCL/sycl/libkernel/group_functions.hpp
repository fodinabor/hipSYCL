...
// Same as HIPSYCL_DISPATCH_BUILTIN/__acpp_backend_switch, except that during an
// SSCP device pass only the SSCP builtin is dispatched: the host group algorithm
// implementations are not compatible with the SSCP compilation flow.
...
#define HIPSYCL_DISPATCH_BUILTIN_2(name, ...)                                  \
  __acpp_backend_switch_2(host_builtins::name(__VA_ARGS__),                    \
                          sscp_builtins::name(__VA_ARGS__),                    \
